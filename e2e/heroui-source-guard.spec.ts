...
test("workspace fallbacks and overlays do not regress to legacy raw shells", () => {
  const forbiddenPatterns: Array<[string, RegExp]> = [
    ...
    ["legacy setup wizard fixed dark terminal surface", /data-raltic-terminal-preview[\s\S]{0,220}bg-zinc-950/],
    ["legacy setup wizard fixed terminal text", /data-raltic-terminal-preview[\s\S]{0,520}text-zinc-300/],
    ...
    ["legacy workspace pane fixed dark terminal surface", /data-raltic-terminal-preview[\s\S]{0,220}bg-zinc-950/],
    ["legacy workspace pane fixed terminal text", /data-raltic-terminal-preview[\s\S]{0,520}text-zinc-200/],
    ["legacy workspace pane raw file preview code block", /<pre className=["'][^"']*bg-\[var\(--surface-secondary\)\]/],
    ...
    ["legacy settings command fixed dark surface", /data-raltic-terminal-command[\s\S]{0,220}bg-zinc-950/],
    ["legacy legal inline code fixed dark surface", /function Code[\s\S]{0,180}bg-zinc-900/],
    ["legacy agent system prompt muted code block", /SystemPromptCard[\s\S]*bg-muted\/50/],
    ["legacy markdown code block muted background", /\.prose-message pre\s*\{[\s\S]{0,180}background:\s*var\(--muted\)/],
    ...
    ["legacy homepage feature black icon chip", /function Feature[\s\S]*color=["']default["'] className=["']h-10 w-10 justify-center p-0 text-zinc-700/],

      ...
      >
        <CardPanel className="mx-auto max-w-5xl px-6 text-center">
          <Chip size="sm" variant="soft" color="default" className="gap-2">
            <Route className="h-3 w-3 text-cyan-400" aria-hidden="true" />
            Workflow templates
          </Chip>
          <h1 className="mx-auto mt-7 max-w-4xl text-balance text-5xl font-medium leading-[1.05] tracking-[-0.02em] text-white sm:text-6xl">
          ...
          <div className="mt-12 grid gap-4 md:grid-cols-2">
            {WORKFLOW_SEO_PAGES.map((page) => (
              <Link
                ...
                href={page.path}
                className="group rounded-xl border border-zinc-200 bg-white p-6 shadow-[0_18px_55px_-38px_rgba(24,24,27,0.42)] transition-colors hover:border-zinc-300 hover:bg-zinc-50"
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <h2 className="mt-4 text-2xl font-medium tracking-tight text-zinc-900">{page.starter.title}</h2>
                  </div>
                  <ArrowRight className="mt-1 h-4 w-4 shrink-0 text-zinc-400 transition-transform group-hover:translate-x-0.5 group-hover:text-zinc-900" aria-hidden="true" />
                </div>
        ...
        <CardPanel className="mx-auto max-w-6xl px-6 py-20">
          ...
          <div className="mt-12 grid gap-3 md:grid-cols-3">
            {[
              ...
              <div key={item} className="rounded-lg border border-zinc-900 bg-zinc-950 p-5">

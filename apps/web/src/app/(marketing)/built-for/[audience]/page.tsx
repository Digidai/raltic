...
export default async function AudienceDetailPage({ params }: Props) {
  ...
  return (
    <>
      ...
      <ContentRouteMap eyebrow="First-value map" title="Start from a concrete team problem and finish with proof a named owner can review." items={page.useCases.map((useCase, index) => ({ label: useCase.title, href: `#use-case-${index + 1}` }))} tone="emerald" />

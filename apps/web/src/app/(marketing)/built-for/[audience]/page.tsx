...
export default async function AudienceDetailPage({ params }: Props) {
  ...
  return (
    <>
      ...
      <section className="border-y border-black/[0.07] bg-[#faf9f6] px-6 py-20"><div className="mx-auto max-w-6xl"><p className="text-center text-xs font-medium uppercase text-emerald-700">Use cases</p><h2 className="mt-4 text-center text-3xl font-medium text-zinc-900 sm:text-4xl">Workflows with a visible first proof.</h2><div className="mt-10 grid gap-4 md:grid-cols-3">{page.useCases.map((useCase) => <article key={useCase.title} className="rounded-lg border border-zinc-200 bg-white p-6"><h3 className="text-lg font-medium text-zinc-900">{useCase.title}</h3><p className="mt-3 text-sm leading-relaxed text-zinc-600">{useCase.body}</p><div className="mt-5 border-t border-zinc-200 pt-4"><p className="text-xs font-medium uppercase text-emerald-700">Reviewable proof</p><p className="mt-2 text-sm leading-relaxed text-zinc-700">{useCase.proof}</p></div></article>)}</div></div></section>

 ...
 *
 * `theme="dark"` (default) is the established treatment used by every
 * marketing page except `/`. `theme="light"` is the ando.so-style light
 * surface used by the redesigned homepage. Both keep the same structure
 * (`[data-raltic-footer-lead]`, `.raltic-marketing-footer-grid`,
 * `.raltic-marketing-footer-base`) so e2e hooks and the lead CTA layout
...
export function MarketingFooter({ lead, leadId, theme = "dark" }: MarketingFooterProps = {}) {
  ...
  return (
    ...
    >
      ...
      <CardPanel className={cn("raltic-marketing-footer-base", isLight && "border-t border-black/[0.07]")}>

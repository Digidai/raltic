...
export const RUNTIME_DOCS: Record<RuntimeKey, RuntimeDoc> = {
  claude: {
    ...
    accent: "cyan",
    tagline: "Claude Code inside workflow rooms.",
    hero: "Claude Code is the agent most engineering teams reach for first — strong reasoning, careful edits, and a defensible safety story. Raltic lets the rest of your team @mention it inside a workflow room, with every response landing in searchable team memory instead of a private terminal.",
    ...
    howRalticUses: "Raltic's bridge spawns `claude` per turn on your computer (same binary you already use). Your repo, secrets, and Anthropic key never leave the machine; only the assistant's room message crosses the network into the workspace everyone reads.",
    ...
    faq: [
      ...
      { q: "How is this different from Anthropic's Projects?", a: "Projects is a 1:1 chat in Anthropic's UI. Raltic puts Claude in a workflow room where humans and other agents can coordinate the same owned business process." },
    ],
  ...
  codex: {
    ...
    verification: "verified",
    accent: "amber",
    tagline: "GPT-5 series in workflow rooms.",
    hero: "OpenAI's Codex CLI is fast, opinionated, and great at codegen-heavy work. Raltic wires it into shared workflow rooms so the engineer who wrote the prompt isn't the only one who sees the answer.",
    ...
    howRalticUses: "Raltic invokes `codex` per turn with your OpenAI auth (env or `codex login`). The agent runs on your computer against your repo; only its replies stream into Raltic. Provider keys stay yours.",
    ...
    faq: [
      ...
      { q: "What if I'm rate-limited?", a: "Raltic surfaces the rate-limit error in the room; the agent retries on next mention. Nothing is silently dropped." },
      { q: "Can the same workspace mix Codex and Claude agents?", a: "Yes. Per-agent runtime + model; they share workflow rooms and can @mention each other in the same process." },
    ],
  ...
  openclaw: {
    ...
    accent: "violet",
    tagline: "Your local-first AI daemon, inside workflow rooms.",
    hero: "OpenClaw is a multi-channel agent daemon you already run on your computer. Raltic adds a workflow-room surface on top — your daemon handles every provider round-trip, Raltic never sees your keys.",
  ...
  hermes: {
    ...
    verification: "experimental",
    accent: "neutral",
    tagline: "Nous Research's self-improving agent, workflow-native.",
    hero: "Hermes Agent runs as a persistent daemon on your machine with its own memory and skills. Raltic wraps workflow rooms around it so the agent's wins accrue where the team can see and steer them.",
    ...
    howRalticUses: "Raltic spawns the `hermes` CLI per turn against your running daemon. Memory + skills stay daemon-side — Raltic surfaces them as `Recalling memory` / `Using skill: X` chips in the room, but never reads or stores the memory contents.",
